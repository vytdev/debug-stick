...
import { Player, Block, BlockStates, world, system } from "@minecraft/server";
...
world.afterEvents.entityHitBlock.subscribe((ev) => {
  ...
  if (player.getComponent("minecraft:inventory").container
    .getItem(player.selectedSlotIndex)?.typeId != "vyt:debug_stick"
  ) return;
...
function changeSelectedProperty(player: Player, block: Block) {
  ...
  let val: BlockStateValue = states[prop];
